...
# Create lagged productivity variable
bartik_full_postdo <- bartik_full_postdo %>%
  group_by(j) %>%
  mutate(firm_productivity_lag2 = lag(firm_productivity_lag), na.rm = TRUE) %>%
  ungroup()
...
ar_x_model <- lm(firm_productivity ~ firm_productivity_lag2, data = bartik_full_postdo)
...
# 6. Estimate kappa
kappa_model <- lm(firm_productivity ~ u_hat + firm_productivity_lag2, data = bartik_full_postdo)
...
# 7. Use Estimated Parameters in Bias Calculations
...
# 7. Function to Calculate Biases
...
# 10. Extract Standard Errors from Bootstrap Results
